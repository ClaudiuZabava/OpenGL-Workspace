...
float clipper_values[] = { 0.0, 0.05, 0.1 };

int colorCode, clipper_poz = 0;
...
myMatrix, resizeMatrix, matrTransl41, matrTransl42, matrTransl1, matrTranslAsphalt, matrTransl3, matrTransl2, matrScaleCars, matrScale2, matrScaleAsphalt, matrScaleBackground, matrRot, matrRot3, matrDepl, matrScaleLights, matrScaleLights2, matrScaleLights3, matrTransl4, matrScaleBeacon1, matrScaleBeacon2, matrRotBeacon1, matrRotBeacon2, matrTransl5, matrTransl6, matrTransl7, matrScaleStops, matrTransl8, matrTransl9, matrTransl10;
...
void RenderFunction(void)
{
    ...
    */
    ...
    matrTransl41 = glm::translate(glm::mat4(1.0f), glm::vec3(i2 + 45, 0.0, 0.0));
    matrTransl42 = glm::translate(glm::mat4(1.0f), glm::vec3(i3 - 45, 120.0, 0.0));

    /*
        Declare the matrix for back ligths
    */
    ...
    matrScaleLights = glm::scale(glm::mat4(1.0f), glm::vec3(2 * clipper_values[clipper_poz], clipper_values[clipper_poz], 0.0));                // utila la desenarea luminilor
    matrScaleLights2 = glm::scale(glm::mat4(1.0f), glm::vec3(0.1, 0.1, 0.0));                // utila la desenarea luminilor
    matrScaleLights3 = glm::scale(glm::mat4(1.0f), glm::vec3(0.1, 0.1, 0.0));                // utila la desenarea luminilor
    ...
    matrRot3 = glm::rotate(glm::mat4(1.0f), -1.57f, glm::vec3(0.0, 0.0, 0.5));
    ...
    */
    ...
    myMatrix = resizeMatrix * matrTransl41 * matrRot * matrScaleLights2;                                  // Matrix for lights car2 = the lower slower one
    colorCode = 3;                                                                              // Set the color
    glUniformMatrix4fv(myMatrixLocation, 1, GL_FALSE, &myMatrix[0][0]);
    glUniform1i(codColLocation, colorCode);
    glDrawArrays(GL_TRIANGLES, 3, 3);

    myMatrix = resizeMatrix * matrTransl42 * matrRot3 * matrScaleLights3;                                  // Matrix for lights car2 = the lower slower one
    colorCode = 3;                                                                              // Set the color
    glUniformMatrix4fv(myMatrixLocation, 1, GL_FALSE, &myMatrix[0][0]);
    glUniform1i(codColLocation, colorCode);
    glDrawArrays(GL_TRIANGLES, 3, 3);
